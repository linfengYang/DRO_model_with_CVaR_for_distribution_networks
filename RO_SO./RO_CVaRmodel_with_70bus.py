...
Qd = [0] * 24  # KVar
for i in range(24): Qd[i] = Pd[i] * 3687.6 / 5385.4  # Qd为Pd的比例
sub = 2
T = 24
ST = sub * T
history = []  # 历史数据
...
def f(Pd,Qd,history,time,PWT,deltapd,Yoe,Yue,pdYoe,pdYuesum):
    ...
    #二阶锥约束
    m.addConstrs((I2[i, t] <= x[i] * 6 ** 2 for i in range(num_branch) for t in range(T)),name='cI1')
    m.addConstrs((I2[i, t] * V2[branch[i, 0] - 1, t] >= Plinehat[i, t] ** 2 + Qlinehat[i, t] ** 2 for i in range(num_branch)
                 for t in range(T)),name='cI2')
    ...
    m.addConstrs((V2[branch[i, 0] - 1, t] - V2[branch[i, 1] - 1, t] <= (1 - x[i]) * MAX + 2 * (
                branch[i, 2] * Plinehat[i, t] + branch[i, 3] * Qlinehat[i, t])
                 - (branch[i, 2] ** 2 + branch[i, 3] ** 2) * I2[i, t] for i in range(num_branch) for t in range(T)),name='c27')
    m.addConstrs((V2[branch[i, 0] - 1, t] - V2[branch[i, 1] - 1, t] >= -(1 - x[i]) * MAX + 2 * (
                branch[i, 2] * Plinehat[i, t] + branch[i, 3] * Qlinehat[i, t])
                 - (branch[i, 2] ** 2 + branch[i, 3] ** 2) * I2[i, t] for i in range(num_branch) for t in range(T)),name='c27_1')
    ...
    ST*=2
    history=np.array(history)
    history_back_low = np.array([min(history[:, i]) for i in range(ST)])
    history_back_high = np.array([max(history[:, i]) for i in range(ST)])

    m.Params.OutputFlag = 0
    m.Params.MIPGAP = 0
    # 目标函数
    m.setObjective(100*gp.quicksum(branch[i, 2] * (I2[i,t])  for i in range(num_branch) for t in range(T))+
                   0.01 * gp.quicksum(z_IL[i, t] for i in range(IL) for t in range(T))+
                   gp.quicksum(lp[i,t] for i in range(DG) for t in range(T))+
                   gp.quicksum(subP[i,t]*history_back_high[i*T+t] for i in range(sub) for t in range(T))
                +gp.quicksum((subP[i,t]+pdYuesum[i-sub,t])*history_back_high[i*T+t] for i in range(sub, 2*sub) for t in range(T))
                   + 80 * sum([Yoe[i, t] for i in range(WT) for t in range(T)]) + 100 * sum(
                [Yue[i, t] for i in range(WT) for t in range(T)])+ 100 * sum([pdYoe[i, t] for i in range(num_node) for t in range(T)]) )
    m.update()
    m.optimize()
    a = m.objVal
    m.dispose()
