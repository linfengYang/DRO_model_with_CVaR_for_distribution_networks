...
def f(Pd,Qd,history,time,PWT,deltapd,Yoe,Yue,pdYoe,pdYuesum,beta):
    ...
    Eph=m.addVars(366,lb=0,vtype=GRB.CONTINUOUS,name='Eph')
    ...
    m.addConstrs(Eph[j]>=100*gp.quicksum(branch[i, 2] * (I2[i,t])  for i in range(num_branch) for t in range(T))+
                ...
                [Yue[i, t] for i in range(WT) for t in range(T)])+ 100 * sum([pdYoe[i, t]
                for i in range(num_node) for t in range(T)]) -alpha for j in range(366))
    ST*=2
    history=np.array(history)
...
for numt in range(len(TIMEpiece)-1):
    ...
    x_1=f( Pd[TIMEstart:TIMEend], Qd[TIMEstart:TIMEend], a, TIMEpiece[numt+1], new_PWT,new_deltapd,new_Yoe,new_Yue,new_pdYoe,new_pdYuesum,beta)
    obj+=x_1
