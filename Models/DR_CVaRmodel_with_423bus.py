...
def funcmain( Pd, Qd, history, time, PWT, deltapd, Yoe, Yue, pdYoe, pdYuesum, beta):
    with open('bus141.txt', 'r') as file:
        f = file.readline()
        bus = []
        while f:
            a = []
            ...
            bus.append(a)
            f = file.readline()
    with open('branch141.txt', 'r') as file:
        f = file.readline()
        branch = []
        while f:
            a = []
    ...
    # 增加节点数
    node141 = len(bus)
    branch141 = len(branch)
    for i in range(node141):
        a = bus[i][:]
        a[0] = bus[i][0] + 141
        bus.append(a)
    for i in range(branch141):
        a = branch[i][:]
        a[0] = branch[i][0] + 141
        a[1] = branch[i][1] + 141
        branch.append(a)
    ...
    for i in range(branch141):
        a = branch[i][:]
        a[0] = branch[i][0] + 141 + 141
        a[1] = branch[i][1] + 141 + 141
        branch.append(a)
    branch.append([52, 69 + 141, 0.012, 0.0091, 0, 0, 0, 0, 0, 0, 0, 0])
    branch.append([52, 69 + 141 * 2, 0.022, 0.011, 0, 0, 0, 0, 0, 0, 0, 0])
    branch.append([107 + 141, 75 + 141 * 2, 0.0374, 0.0214, 0, 0, 0, 0, 0, 0, 0, 0])
    branch.append([2 + 141, 36 + 141 * 2, 0.0474, 0.0314, 0, 0, 0, 0, 0, 0, 0, 0])
    bus = np.array(bus)
    ...
    WT = 6  # 风电台数
    WT_bus = [35, 58, 35 + 141, 58 + 141, 35 + 141 * 2, 58 + 141 * 2]  # 风电位置
    WTmax = 800 / (MVAbase * 1e3)  # 额定输出
    WTfactor = 0.80
    WT_r = WTmax * WTfactor  # 额定有功输出

    DG = 6  # DG数
    DG_bus = [80, 110, 80 + 141, 110 + 141, 80 + 141 * 2, 110 + 141 * 2]  # DG位置
    DGfactor = 0.80
    DGPmax = 2000 / (MVAbase * 1e3)
    DGPmin = 200 / (MVAbase * 1e3)
    DGcost = [[0.575, 159.325, 0], [0.725, 158.875, 0], [0.775, 157.325, 0],
              [0.525, 159.875, 0], [0.675, 158.325, 0], [0.425, 157.875, 0]]
    for i in range(DG):
        DGcost[i][0] *= MVAbase
    ...
    SVCmax = 800  # KVar
    SVCmax /= (MVAbase * 1e3)
    SVC_bus = [61, 73, 61 + 141, 106, 73 + 141, 61 + 141 * 2, 105 + 141 * 2, 73 + 141 * 2, 130 + 141,
               130 + 141 * 2]  # 补偿器位置

    # 支路容量
    Plinemax = 10000 / (MVAbase * 1e3)  # KW
    Qlinemax = 8000 / (MVAbase * 1e3)  # KVar
    ...
    T = time
    sub = 6
    sub_bus = [1, 85, 1 + 141, 85 + 141, 1 + 141 * 2, 85 + 141 * 2]
    ST = sub * T
    num_branch = len(branch)  # 分支数量
    num_node = len(bus)  # 节点数量
    MAX = 10  # 大数
    for i in range(T):
        Pd[i] *= 35833 / 5385  # 35833.8是423模型的总有功
        Qd[i] *= 22207 / 3687  # 22207.8是423模型的总无功
        Pd[i] /= (MVAbase * 1e3)
        Qd[i] /= (MVAbase * 1e3)
    ...
    np.random.seed(678)
    IL = 9
    ILbus = [64, 112, 98, 64 + 141, 112 + 141, 98 + 141, 64 + 141 * 2, 112 + 141 * 2, 98 + 141 * 2]  # 可中断负荷bus
    ILrate = 0.3  # 可中断负荷占比
    totalIL = [0] * T  # 总可中断负荷
    for t in range(T):
        temp = 0
    ...
    for i in range(IL):
        for t in range(T):
            ...
            IL_alpha[i][t] = MVAbase * 0.625 * (1 - Pd[t] / (35833) * 0.1) + np.random.normal(0 + somean * 2,0.02 + soscale * 1.5)
            IL_beta[i][t] = 150 * (1 - Pd[t] / (22207) * 0.1) + np.random.normal(0 + somean * 1e3, 3 + soscale * 2e2)
    m = gp.Model()
    ...
    QIL = m.addVars(IL, T, vtype=GRB.CONTINUOUS, name='QIL')  # 可中断负荷的中断值
    ...
    # 二阶锥约束
    m.addConstrs(I2[i, t] <= x[i] * 2 ** 2 for i in range(num_branch) for t in range(T))
    m.addConstrs(I2[i, t] * V2[branch[i, 0] - 1, t] >= Plinehat[i, t] ** 2 + Qlinehat[i, t] ** 2 for i in range(num_branch)
                 for t in range(T))
    ...
    # 电压约束
    m.addConstrs((V2[(sub_bus[i] - 1), t] == 1.05 ** 2 for i in range(sub) for t in range(T)), name='c28')
    ...
    m.addConstrs(V2[branch[i, 0] - 1, t] - V2[branch[i, 1] - 1, t] >= -(1 - x[i]) * MAX + 2 * (
                 ...
                 - (branch[i, 2] ** 2 + branch[i, 3] ** 2) * I2[i, t] for i in range(num_branch) for t in range(T))
    m.addConstrs(
        (((2 * DGcost[i][0] * DGproduct_l[i][l] + DGcost[i][1]) * PM[(DG_bus[i] - 1), t] + DGcost[i][2] -
    ...
    m.Params.MIPGap = 0
    m.setObjective(eta + gp.quicksum(omega[i] * gama[i] for i in range((L1) * ST)) + gp.quicksum( epi[i] * mean[i] for i in range(ST)))  # 目标函数
    m.update()
    m.optimize()
    a = m.objVal
    m.dispose()
