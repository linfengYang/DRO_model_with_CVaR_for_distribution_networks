...
def funcpd(beta ):
    ...
    Qd = [0] * 24
    for i in range(24):Qd[i]=Pd[i]*3687.6/5385.4 # 无功需求与有功的比例
    sub=6
    T=24
    #读取历史数据
    ...
    for i in range(length):
        for j in range(T):
            ...
            for k in range(1,sub):
                history[i,j+k*T]=history[i,j]+np.random.normal(0,1)
    ...
    def f_pd(Pd,Qd,history,time,beta,delta,flag):
        ...
        bus[:,2]=bus[:,2]*pf
        if flag: bus=bus[7:8] # 选取节点系统中第一个有负荷需求的点
        Vbase = 12.47 # 基准电压
        MVAbase = 10  # 基准视在功率
        ...
        #扩大负荷，避免精度误差
        for i in range(T):
            Pd[i] *= 1000*35833/5385 # 35833.8是模型的总有功负荷
            Qd[i] *= 1000*22207/3687 # 22207.8是模型的总无功负荷
            Pd[i] /= (MVAbase * 1e3)
            Qd[i] /= (MVAbase * 1e3)
        ...
        if flag:
            pd = [[i * j /(35833)*63.75 for i in Pd] for j in pd_factor]  # 获得节点各个时段的有功负荷,63.75为该节点的有功
            qd = [[i * j /(22207)*39.5for i in Qd] for j in qd_factor]
        else:
        ...
        deltaYuesum = m1.addVars(sub, T, vtype=GRB.CONTINUOUS, name='deltaYuesum')  # 将负荷低估加起来
        ...
        s = [x_low, x_high]
        C_ = []
        for i in range(ST): C_.extend([x_low[i] + (x_high[i] - x_low[i]) * l / L1 for l in range(L1 + 1)])
        C_ = np.array(C_)
        gama=[]
        for i in range(ST):
            for l in range(L1+1):
        ...
        m1.addConstrs((gp.quicksum(C[j, i] * pi1[j] for j in range(G)) == 0 for i in range(ST)),name="c30")
        m1.addConstrs((gp.quicksum(D[j, i] * pi1[j] for j in range(G)) == -omega[i] for i in range((L1 + 1) * ST)),name="c31")
        ...
        m1.Params.MIPGap = 0
        m1.setObjective( eta + gp.quicksum(omega[i] * gama[i] for i in range((L1) * ST)))  # 目标函数
        m1.update()
        m1.optimize()
    ...
    TIMEpiece=[0]
    TIMEpiece.extend([1for i in range(T)])
    b=history[:,1::T]
    TIMEstart=0
    TIMEend=TIMEpiece[0]
    num_node=141*3
    ...
    for numt in range(len(TIMEpiece)-1):
        ...
        S = []
        DELTA_1 = [0.20+  (beta-0.9)  for t in range(T)]
        S_pd=[]
        for n in range(60):
            s = 0
            DELTA1 = DELTA_1[:]
            DELTA1[numt] = DELTA1[numt] + n * (0.02+(0.95-beta)**1.5) * DELTA_1[numt]
    ...
    for t in range(T):
        S_scale[t]=S_scale[t]/0.23558496*Pd[0]/Pd[t]
    ...
    for numt in range(len(TIMEpiece)-1):
        ...
        for i in range(num_node):
            deltapd_copy[i].append(result[0][i]/1000) #除以乘的倍数
