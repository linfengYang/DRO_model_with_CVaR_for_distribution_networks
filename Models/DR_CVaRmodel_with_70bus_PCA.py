...
def funcmain( Pd, Qd, history, time, PWT, deltapd, Yoe, Yue, pdYoe, pdYuesum, beta, Gslidenumber):
    ...
    # 支路容量
    Plinemax = 5000 / (MVAbase * 1e3)
    Qlinemax = 4000 / (MVAbase * 1e3)
    ...
    subP = m.addVars(2 * sub, T, vtype=GRB.CONTINUOUS, name='subP')  # 变电站da与rt发电
    ...
    numIL_L = 5  # 可中断负荷分段
    IL_L = [[[l * (ILrate * pd[ILbus[i] - 1, t]) / numIL_L for l in range(numIL_L + 1)] for t in range(T)] for i in range(IL)]
    IL_L = np.array(IL_L)
    ...
    m.addConstrs((((2 * DGcost[i][0] * DGproduct_l[i][l] + DGcost[i][1]) * PM[(DG_bus[i] - 1), t] + DGcost[i][2] -
                   DGcost[i][0] * (DGproduct_l[i][l] ** 2)) <= lp[i, t] for i in range(DG) for t in range(T) for l in
                  range(lpL + 1)), name="c11")

    history = np.array(history)
    M = 366
    # PCA降维
    miu = np.mean(history, axis=0)
    std = np.std(history, axis=0)
    history = (history - miu) / std
    ...
    Gslide =  sub * T
    projection_matrix = eigenvectors[:, :Gslide]
    history_slide = np.matmul(history, projection_matrix)
    projection_matrix = (projection_matrix.T * std).T

    history_back = np.matmul(history_slide, projection_matrix.T) + miu
    GT, L1 = 2 * ST, 30
    history_back_low = np.array([min(history_back[:, i]) for i in range(GT)])
    history_back_high = np.array([max(history_back[:, i]) for i in range(GT)])
    C_slide = []
    phi_number = Gslide
    for i in range(phi_number):
        C_slide.extend([history_back_low[i] + (history_back_high[i] - history_back_low[i]) * l / L1 for l in range(1, L1 + 1)])
    C_slide = np.array(C_slide)
    gama = [sum([max(history_back[j][i] - C_slide[i * (L1) + l], 0) for j in range(M)]) / M + 1 for i in
            range(phi_number) for l in range(L1)]

    # 模糊集矩阵化
    C1 = np.zeros((GT, Gslide))
    C2 = np.zeros((GT, Gslide))
    C3 = np.zeros((phi_number, Gslide))
    C4 = [C1, C2]
    [C4.append(C3) for _ in range(2 * L1)]
    C = np.concatenate(C4, axis=0)
    k = 0
    for i in range(GT):
        C[i] = projection_matrix[i]
        C[i + GT] = -projection_matrix[i]
    for i in range(L1 * phi_number + 2 * GT, 2 * L1 * phi_number + 2 * GT):
        C[i] = projection_matrix[k]
        if not (i - L1 * phi_number - 2 * GT + 1) % L1:
            k += 1
    D1 = np.diag([-1] * (L1) * phi_number)
    D = np.vstack(
        (np.zeros((2 * GT, (L1) * phi_number)), D1, D1))
    d = np.zeros(((L1) * phi_number))
    d = np.hstack((d, np.zeros((2 * GT)), d))
    d[:GT] = history_back_high - miu
    d[GT:2 * GT] = miu - history_back_low
    C_slide_miu = np.zeros((phi_number * (L1)))
    k = 0
    for i in range(phi_number * (L1)):
        C_slide_miu[i] = C_slide[i] - miu[k]
        if not (i + 1) % L1:
            k += 1
    d[L1 * phi_number + 2 * GT:] = C_slide_miu[:]
    G = 2 * L1 * phi_number + 2 * GT

    # 对偶化问题
    eta = m.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name="eta")
    omega = m.addVars((L1) * phi_number, vtype=GRB.CONTINUOUS, name="omega")
    epi = m.addVars(GT, lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name="epi")
    pi1 = m.addVars(G, vtype=GRB.CONTINUOUS, name="pi1")
    m.addConstr(-gp.quicksum(d[i] * pi1[i] for i in range(G)) + eta - alpha >= 0, name="c12")
    m.addConstrs((gp.quicksum(C[j, i] * pi1[j] for j in range(G)) == -gp.quicksum(
                epi[j] * projection_matrix[j, i] for j in range(GT)) for i in range(Gslide)), name="c13")
    m.addConstrs((gp.quicksum(D[j, i] * pi1[j] for j in range(G)) == -omega[i] for i in range((L1) * phi_number)),name="c14")
    ...
    m.addConstr(-gp.quicksum(d[i] * pi2[i] for i in range(G)) + eta + (beta * alpha
                ...
                - gp.quicksum((subP[i, t]) * miu[i * T + t] for i in range(2 * sub) for t in range(T))
                - gp.quicksum(pdYuesum[i, t] * miu[(sub + i) * T + t] for i in range(sub) for t in range(T))
    ...
    m.addConstrs((gp.quicksum(C[j, i] * pi2[j] for j in range(G)) ==
                  (gp.quicksum((subP[j, t]) * projection_matrix[j * T + t, i] for j in range(sub) for t in range(T)) +
                   gp.quicksum((subP[j, t] + pdYuesum[j - sub, t]) * projection_matrix[j * T + t, i] for j in
                   range(sub, 2 * sub)for t in range(T))) / (1 - beta) - gp.quicksum(epi[j] * projection_matrix[j, i]
                   for j in range(GT)) for i in range(Gslide)), name="c17")
    m.addConstrs((gp.quicksum(D[j, i] * pi2[j] for j in range(G)) == -omega[i] for i in range((L1) * phi_number)), name="c18")

    m.Params.MIPGAP = 0
    m.Params.OutputFlag = 0
    m.setObjective(eta + gp.quicksum(omega[i] * gama[i] for i in range((L1) * phi_number)))
    m.update()
    m.optimize()
    a = m.objVal
    m.dispose()
