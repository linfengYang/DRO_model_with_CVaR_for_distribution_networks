import datetime
import numpy as np
from DR_CVaRmodel_WT_output_with433bus import funcwt
from DR_CVaRmodel_power_load_with433bus import funcpd
from DR_CVaRmodel_with_423bus import funcmain
...
for i in range(length - 24, length):
    for j in range(T):
        ...
        for k in range(1, sub):
            history[i, j + T*k] = history[i, j] + np.random.normal(0, 1)
for i in range(length):
    for j in range(T):
        history[i, j + sub * T] = history_rt[i * T + j]
        for k in range(1, sub):
            history[i, j + sub * T + T*k] = history[i, j + sub * T]+ np.random.normal(0, 1)
